document.addEventListener('DOMContentLoaded', function () {
  ...
  (function initErp(){
    ...
    var erpRebuildRunning = false;
    if (rebuildAllBtn) rebuildAllBtn.addEventListener('click', function(){
      rebuildAllBtn.disabled = true; if (abortBtn) abortBtn.disabled = false; erpRebuildRunning = true;
      ...
      fetch(nsBase + '/movements/rebuild-all/start', { method: 'POST', headers: { 'X-WP-Nonce': data.nonce } })
        ...
        .catch(function(){ rebuildAllBtn.disabled = false; if (abortBtn) abortBtn.disabled = true; erpRebuildRunning = false; });
      function stepLoop(){
        if (!erpRebuildRunning) return;
        fetch(nsBase + '/movements/rebuild-all/step', { method: 'POST', headers: { 'X-WP-Nonce': data.nonce } })
          .then(function(r){ return r.json(); })
          .then(function(state){
            setProg(state.processed||0, state.total||0);
            if (state.status === 'aborted') { erpRebuildRunning = false; rebuildAllBtn.disabled = false; if (abortBtn) abortBtn.disabled = true; return; }
            if (state.status === 'done' || (state.processed>=state.total)) { erpRebuildRunning = false; rebuildAllBtn.disabled = false; if (abortBtn) abortBtn.disabled = true; load(); return; }
            setTimeout(stepLoop, 200);
          })
          .catch(function(){ erpRebuildRunning = false; rebuildAllBtn.disabled = false; if (abortBtn) abortBtn.disabled = true; });
      }
    });
    if (abortBtn) abortBtn.addEventListener('click', function(){
      if (abortBtn.disabled) return;
      var nsBase = data.restBase.replace(/\\/product\\/\\d+$/, '');
      abortBtn.disabled = true; erpRebuildRunning = false;
      fetch(nsBase + '/movements/rebuild-all/abort', { method: 'POST', headers: { 'X-WP-Nonce': data.nonce } })
        .then(function(){ rebuildAllBtn && (rebuildAllBtn.disabled = false); })
        .catch(function(){ rebuildAllBtn && (rebuildAllBtn.disabled = false); });
    });
  })();
});
